...
mean['ATR'] = [0, 0, 0]
mean['CUB'] = [0.485, 0.456, 0.406]


std['CUB'] = [0.229, 0.224, 0.225]
...
def get_transform_cub(train):
    if train:
        transform = transforms.Compose([
            ...
            transforms.RandomCrop(224, padding=4, padding_mode='reflect'),
            transforms.ToTensor(),
            transforms.Normalize(mean["CUB"], std["CUB"])])
    else:
        transform = transforms.Compose([
            transforms.Resize([224, 224]),
            transforms.ToTensor(),
...
class SSL_Dataset:
    ...
    def __init__(self,
        ...
        crop_size = 96 if self.name.upper() == 'STL10' else 224 if (self.name.upper() == 'IMAGENET' or self.name.upper() == 'CUB')  else 64 if (self.name.upper() == 'ATR' or self.name.upper() == 'FLIR') else 32
        if self.name.upper() == 'CUB':
            self.transform = get_transform_cub(train)
        else:
    ...
    def get_flir_data(self,txt_data_dir, lb_files):
        ...
        data, targets = dataset["images"].transpose([0,2,3,1]).astype('float32'), dataset["labels"]
        data, targets = data.astype(np.uint8), targets.astype(np.int64)
        print(data.shape)
    ...
    def get_dset(self, is_ulb=False,
        ...
        elif self.name.upper() in  ['FLIR', 'CUB']:
