...
netG.load_state_dict(torch.load(r'C:\Users\user\Desktop\TorchSSL-main\trained_vaes\cifar10_dcgan\weights\netG_epoch_199.pth'))
...
netD.load_state_dict(torch.load(r'C:\Users\user\Desktop\TorchSSL-main\trained_vaes\cifar10_dcgan\weights\netD_epoch_199.pth'))
...
for epoch in range(50):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        iters += 1
print("--- Running time (VAE) =  %s seconds ---" % (time.time() - start_time))
...
fake.shape
Xn = 255*(fake - fake.min(axis=(1,2,3))[:,None,None,None])/(fake.max(axis=(1,2,3))[:,None,None,None] - fake.min(axis=(1,2,3))[:,None,None,None])
Xn.min()
Xn.shape
Y_ = np.array([-1]*7352)
data_set = {}
...
exp_id='35tr_s2_GAN'
import h5py
h = h5py.File('./data/{}.hdf5'.format('synth_ul_'+exp_id), 'w')
