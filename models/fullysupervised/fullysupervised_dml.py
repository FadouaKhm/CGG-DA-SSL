...
class FullySupervised:
    ...
    def train(self, args, loss_func, mining_func):
        ...
        for _, x_lb, y_lb in self.loader_dict['train_lb']:
            ...
            with amp_cm():
                ...
                if args.net == 'vgg16_bn':
                    out_aux = self.model.features(x_lb)
                    out_aux = self.model.avgpool(out_aux)
                    out_aux = torch.flatten(out_aux,1)
                    logits_x_lb = self.model.classifier(out_aux) 
                    embeddings = self.model.classifier[0](out_aux)
                    
                else:
                    logits_x_lb, embeddings = self.model(x_lb,  ood_test=True)

                indices_tuple = mining_func(embeddings, y_lb)
                dml_loss = loss_func(embeddings, y_lb, indices_tuple)

                sup_loss = ce_loss(logits_x_lb, y_lb, reduction='mean')

                # total_loss = sup_loss
                total_loss = dml_loss + sup_loss
            ...
            tb_dict['train/dml_loss'] = dml_loss.detach()
