...
from utils.train_utils import ce_loss, wd_loss, EMA, Bn_Controller
...
class FullySupervised:
    def __init__(self, net_builder, num_classes,
        ...
        #     self.model = net_builder(pretrained=True)
        #     self.model.classifier[6] = nn.Linear(4096, num_classes)
        # elif net_name == 'resnet50' or net_name == 'resnet34':
        #     self.model = net_builder(pretrained=True)
    ...
    def train(self, args):
        ...
        for _, x_lb, y_lb in self.loader_dict['train_lb']:
            ...
            start_run.record()
            x_lb = x_lb.to(device) #x_lb.cuda(args.gpu)
            y_lb = y_lb.to(device) #y_lb.cuda(args.gpu)
